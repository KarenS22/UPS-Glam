...
import com.instagram.backend.dto.PublicationFeedDto;
...
import com.instagram.backend.repository.PublicationRepository;
import com.instagram.backend.service.SupabaseStorageService;
import com.instagram.backend.dto.ProcessedPublicationRequest;
...
public class PublicationController {
    ...
    public Mono<Publication> createProcessedPublication(@RequestBody ProcessedPublicationRequest request) {
        ...
        return ReactiveSecurityContextHolder.getContext()
                ...
                .flatMap(userId -> {
                    ...
                    log.info("Saving publication details with GPU filter to database for user: {}", userId);
                ...
                .flatMap(userId -> 
                    ...
                    filePartToBytes(filePart)
                            .flatMap(bytes -> {
                                ...
                                String fileName = "publications/" + storageService.generateUniqueFileName(extension);
                                ...
                                return storageService.uploadImage(bytes, fileName, "image/jpeg")
                                        .flatMap(url -> {
                                            ...
                                            log.info("Saving publication details to database for user: {}", userId);
                                            return publicationRepository.save(publication);
                                        });
                            })
                );
        ...
        return ReactiveSecurityContextHolder.getContext()
                ...
                .flatMapMany(currentUserId -> 
                        publicationRepository.findAllByOrderByCreatedAtDesc()
                                .flatMap(pub -> {
                                    Mono<Profile> creatorMono = profileRepository.findById(pub.getUserId())
                                            .defaultIfEmpty(new Profile(pub.getUserId(), "deleted_user", "Deleted User", null, null, false));
                                    Mono<Long> likesCountMono = likeRepository.countByPublicationId(pub.getId());
                                    Mono<Boolean> likedByMeMono = likeRepository.existsByPublicationIdAndUserId(pub.getId(), currentUserId);

                                    // Zip all data points to compile a unified feed DTO
                                    return Mono.zip(creatorMono, likesCountMono, likedByMeMono)
                                            .map(tuple -> PublicationFeedDto.builder()
                                                    .publication(pub)
                                                    .creator(tuple.getT1())
                                                    .likesCount(tuple.getT2())
                                                    .isLikedByMe(tuple.getT3())
                                                    .build()
                                            );
                                })
                );
    ...
    public Flux<PublicationFeedDto> getUserPublications(@PathVariable UUID userId) {
        ...
        return ReactiveSecurityContextHolder.getContext()
                .map(ctx -> (UUID) ctx.getAuthentication().getPrincipal())
                .flatMapMany(currentUserId -> 
                        publicationRepository.findAllByUserIdOrderByCreatedAtDesc(userId)
                                .flatMap(pub -> {
                                    Mono<Profile> creatorMono = profileRepository.findById(pub.getUserId())
                                            .defaultIfEmpty(new Profile(pub.getUserId(), "deleted_user", "Deleted User", null, null, false));
                                    Mono<Long> likesCountMono = likeRepository.countByPublicationId(pub.getId());
...
log.info("Deleting publication {} by owner {}", id, currentUserId);
// 1. Delete comments and likes first to avoid referential integrity errors
Mono<Void> deleteComments = commentRepository.deleteAllByPublicationId(pub.getId());
Mono<Void> deleteLikes = likeRepository.deleteAllByPublicationId(pub.getId());
// 2. Delete original and processed images from storage
Mono<Void> deleteOriginalImage = storageService.deleteImage(pub.getImageUrl());
Mono<Void> deleteProcessedImage = pub.getProcessedImageUrl() != null && !pub.getProcessedImageUrl().isEmpty()
? storageService.deleteImage(pub.getProcessedImageUrl())
: Mono.empty();
// 3. Delete publication
Mono<Void> deletePub = publicationRepository.deleteById(pub.getId());
// Chain everything: relations, storage, then parent entity
return Mono.when(deleteComments, deleteLikes)
.then(Mono.when(deleteOriginalImage, deleteProcessedImage))
.then(deletePub);
} else {
